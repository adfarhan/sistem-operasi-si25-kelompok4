...
## ðŸ‘¥ Anggota Kelompok 4 (Kelas SI-2025A / SI-2025B)
...
## ðŸŽ¯ Spesifikasi Lingkungan Server
...
## ðŸ› ï¸ Langkah-Langkah & Dokumentasi Praktikum
...
* Setelah reboot, login sebagai `root`, membuat user biasa, lalu menambahkannya ke grup sudo:
  ...
  **Penjelasan tiap perintah:**
  ...
  - `apt install sudo -y` memasang paket `sudo`, yang dibutuhkan agar user biasa nantinya bisa menjalankan perintah administratif tanpa harus login penuh sebagai `root` â€” praktik ini lebih aman karena akses root hanya dipinjam sementara per-perintah.
...
* Menginstal paket pendukung:
  ...
  **Penjelasan tiap paket:**
  - `net-tools` â€” menyediakan perintah jaringan klasik seperti `ifconfig`, `netstat` untuk membantu diagnosa jaringan.
  - `curl` â€” dipakai untuk mengirim request HTTP langsung dari terminal, berguna untuk menguji web server tanpa perlu browser (dipakai di langkah berikutnya).
  - `git` â€” sistem kontrol versi, berguna bila nanti perlu mengambil/mengelola kode dari repository.
...
* Menginstal, menjalankan, dan mengaktifkan Nginx agar berjalan otomatis saat booting:
  ...
  **Penjelasan tiap perintah:**
  ...
  - `sudo systemctl enable nginx` membuat Nginx otomatis menyala setiap kali sistem melakukan boot, tanpa perlu dijalankan manual tiap kali â€” penting untuk server yang harus selalu online.
...
* Halaman *Welcome to nginx!* berhasil tampil melalui `curl`, menandakan service Nginx berjalan dengan baik.
  ...
  **Penjelasan:** Output HTML default ini adalah halaman bawaan Nginx yang tersimpan di `/var/www/html/index.html`. Keberhasilan menampilkannya membuktikan seluruh rantai proses â€” instalasi paket, aktivasi service, hingga konfigurasi default â€” berjalan tanpa masalah.
...
## ðŸŒ Port Forwarding - Akses dari Host

* Mengecek IP Address Debian (Guest) dengan perintah `ip a`, mencatat alamat IP pada antarmuka *ens33*.

  **Penjelasan:** Karena VM berjalan dalam mode **NAT**, VM memiliki IP tersendiri yang berbeda dari IP komputer host. IP inilah yang perlu dicatat agar bisa didaftarkan pada aturan port forwarding di langkah berikutnya â€” tanpa IP yang benar, VMware tidak tahu VM mana yang harus dituju.

* Melakukan pemetaan **Host Port 8080** ke **VM Port 80** melalui menu `Edit` â†’ `Virtual Network Editor` â†’ pilih **VMnet8 (NAT)** â†’ **NAT Settings** â†’ **Add** pada bagian Port Forwarding, dengan mengisi alamat IP Guest yang sudah dicatat.
...
## ðŸ“ Kesimpulan
Tuliskan simpulan dari praktikum yang telah dilakukan serta poin-poin penting yang didapatkan selama melakukan setup server Linux headless, misalnya pemahaman instalasi Debian mode teks, konfigurasi user sudo, pengelolaan service Nginx, serta konsep port forwarding pada hypervisor.
